...
pub async fn create_id_photo(
    params: JsonBody<CreateIdPhotoRequest>,
    depot: &mut Depot,
    res: &mut Response,
) -> AppResult<()> {
    let state = depot.obtain::<AppState>().map_err(|_| StatusError::internal_server_error())?;
    ...
    let image: Vec<u8> = redis.get(&req.image_id).await.unwrap();

    let item = photo_items::Model::find_by_id(&state.conn, req.item_id.deref()).await?;
...
pub async fn update_id_photo(
    params: JsonBody<UpdateIdPhotoRequest>,
    depot: &mut Depot,
    res: &mut Response,
) -> AppResult<()> {
    let state = depot.obtain::<AppState>().map_err(|_| StatusError::internal_server_error())?;
    ...
    let client = reqwest::Client::new();
    let form = Form::new()
        .text("render", req.render.to_string())
        .text("color", req.color)
...
pub async fn delete_photo(
    ...
    photos::Model::delete_user_photo_by_id(&state.conn, req.image_id.as_str(), user.id.as_str())
        .await?;
